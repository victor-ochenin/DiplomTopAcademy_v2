  ...
  if (progress) {
    for (const lid of course.lessonIds) {
      for (const key of Object.keys(progress.completedTasks)) {
        if (key.startsWith(lid + ':')) { completedItems++ }
      }
    }
  }
  const percent = course.itemsCount > 0 ? Math.round((completedItems / course.itemsCount) * 100) : 0

  const iconEl = course.icon
    ? <div className="course-icon" dangerouslySetInnerHTML={{ __html: course.icon }} /* SVG из своих JSON, доверенный */ />
    : <div className="course-icon course-icon--fallback">{course.title.slice(0, 2)}</div>
      ...
      <div className="card" onClick={onEnter}>
        ...
        <div className="card-footer">
          <div className="footer-item">
            ...
            <span>{course.lessonCount} {pluralize(course.lessonCount, 'урок', 'урока', 'уроков')}</span>
          </div>
          <div className="footer-item">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
            </svg>
            <span>{course.itemsCount} {pluralize(course.itemsCount, 'задание', 'задания', 'заданий')}</span>
          </div>
        </div>
      </div>
    </div>
