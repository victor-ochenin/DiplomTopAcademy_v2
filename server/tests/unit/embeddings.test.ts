...
describe('OpenRouterEmbeddingFunction', () => {
  it('sends correct request body and headers', async () => {
    fetchMock.mockResolvedValue(
      mockResponse({ data: [{ index: 0, embedding: [0.1] }] }),
    )
  ...
  it('throws on non-ok status', async () => {
    ...
    const emb = new OpenRouterEmbeddingFunction({ apiKey: 'sk-test' })
    await expect(emb.generate(['x'])).rejects.toThrow(
      'OpenRouter embedding failed (429)',
    )
  })
  ...
  it('throws when data is missing', async () => {
    ...
    const emb = new OpenRouterEmbeddingFunction({ apiKey: 'sk-test' })
    await expect(emb.generate(['x'])).rejects.toThrow(
      'OpenRouter returned invalid embedding response',
    )
  })
  ...
  it('throws when indexes are incomplete or duplicated', async () => {
    ...
    const emb = new OpenRouterEmbeddingFunction({ apiKey: 'sk-test' })
    await expect(emb.generate(['a', 'b'])).rejects.toThrow(
      'OpenRouter returned incomplete embedding response',
    )
  })
